...
def start_ocp(user_id: str, channel: str, client: WebClient, text: str):
    ocp = OCP(channel)
    message =  ocp.oc_exec(text)
    response = client.chat_postMessage(**message)

def start_flipacoin(user_id: str, channel: str, client: WebClient):
    flip_a_coin = CoinBot(channel)
    message =  flip_a_coin.get_message_payload()
    response = client.chat_postMessage(**message)
...
def message(event, client):
    ...
    if "hey sammy, flip a coin" in text.lower():
        return start_flipacoin(user_id, channel_id, client)
    if "ocplab" in text.lower():
        return start_ocp(user_id, channel_id, client, text)
